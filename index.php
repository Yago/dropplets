    ...
    if($posts) {
        ...
        foreach($posts as $post) {
            if($c<$feed_max_items) {
                ...
                $item->setDescription(Parsedown::instance()->parse(implode($remove_metadata_from)));
...
/*-----------------------------------------------------------------------------------*/
    ...
    // If there is a file for the selected permalink, display and cache the post.
        ...
        $post_content = Parsedown::instance()->parse(trim(implode("", $file_array)));
        ...
        // Generate the post.
        $post = Parsedown::instance()->parse(join('', $fcontents));
